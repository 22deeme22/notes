\section{Quantum key distribution (QKD)}
    ...
    \subsection{BB84 protocol}
        ...
        \end{tcolorbox}
            ...
            \[\ket{\psi_{x_1}}=H^{e_i}\ket{x_1} \text{ where $e_i$ $\sim$ Unif \{0,1\}} \implies \ket{\psi_{\vec{x}}}= \bigotimes_{j=0}^{m} H^{e_i}\ket{x_j}.\]
            
            \item \textbf{Step Decoding:}

                Bob recieves $\ket{\psi_{\vec{x}}}=\bigotimes_{j=1}^{m} H^{e_i}\ket{x_j}$, he chooses a random basis to measure it. If Bob know the basis, he can decode easily, he doesn't!
                \begin{itemize}[left=10pt, label=\textbullet]
      ...
      \]
            ...
            Maybe sometimes bits of Alice and Bob can disagree not because of an eavesdropper but because of \important{noise}! To deal with noise we can use some classical error correction.
