...
\subsection{Language level consistency}
    \begin{tcolorbox}[vert]
        A \important{data race} is when two conflicting operations from different thrads occur simultaneously. Operations conflict if they access the same location, and at least one is a write and these simultaneous operations are defined back-to-back accesses in any sequentially consistency.
    \end{tcolorbox}
...
Unlock:
    ...
    \begin{tcolorbox}[rouge]
        The idea behind ``test-and-test-and-set'' (TTS) is to just read classicaly the lock, if it's 1 (locked), then we juste wait. Then, only if it's 0, we read one more time the lock with the TS instruction. So when the processor is waiting, it juste does BusRd instead of BusRdX and doesn't invalidate other caches.
        \begin{codeCenter}
        \begin{Ccode}
Test:
...
synchronized(tbl) { 
    ...
    \subsubsection{Detection}
        Sometimes we can't prevent deadlocks, then, the second best approach is to detect it and kill procceses involved in the deadlock. This approach require two things: an algorithm to detect deadlocks and an algorithm to recover from a deadlock.
        \begin{tcolorbox}[vert]
            A \important{resource allocation graph (RAG)} is a graph where: nodes are either resources or threads, a resource node can have multiple units and there is directed edges between threads and resource. If RAG has no directed cycle, the current state is deadlock-free. If RAG has a directed cycle: if there is only one unit per resource, there is a deadlock, for multiple units per resource, there is a possibiliy of deadlock.
        \end{tcolorbox}
    ...
    \subsection{Transactional memory}
        ...
        \begin{tcolorbox}[gris]
            Let's introduce three Intel Haswell intructions: \lstinline!xbegin! \textrightarrow start transaction, takes pointer to fallback address in case of abors, \lstinline!xend! \textrightarrow ends transaction, \lstinline!xabort! software initiated transaction abordt
        \end{tcolorbox}
...
\end{codeCenter}\hypertarget{type0}{}
    ...
    \end{enumerate}
    The context saved means that we that we save the ``state'' of a thread that must be saved before a switch.
