...
\subsection{Iron Law}
    \begin{tcolorbox}[rouge]
        \[\text{Execution time}= \frac{1}{\text{Processor performance}}= \frac{\text{Instructions}}{\text{Program}} \cdot \frac{\text{Cycles}}{\text{Instruction}} \cdot \frac{\text{Seconds}}{\text{Cycle}}.\]
    \end{tcolorbox}
    \begin{itemize}[left=10pt, label=\textbullet]
        \item \textbf{$\frac{\text{Instructions}}{\text{Program}}$}:
            \begin{tcolorbox}[gris]
                ...
                \end{itemize}
                \begin{remark}{Remark}
                    The runtime system is a program given with the programmation language (ex. JVM for Java), that gives necessary services and mechanisms to the program during its execution, like memory allocation, etc\ldots
                \end{remark}
            \end{tcolorbox} 
        \item \textbf{$\frac{\text{Cycles}}{\text{Instruction}}$}:
            \begin{tcolorbox}[gris] The $\frac{\text{Cycles}}{\text{Instruction}}$ part represent the number of cycles by instructions, it is influenced by: \begin{itemize}[left=10pt] \item The front-end cycles (instruction cache, branch predictor, etc\ldots), \item The back-end cycles (data cache, ALU, FPU, etc\ldots), \item  The memory (latency, bandwidth).
                \end{itemize}
                \begin{remark}{Remark}
                    Front-end part of the pipeline CPU is the instruction fetching, decoding and branch prediction part, and the back-end is the computation, memory access and data storage part.
                    
                    We have to be careful! The cycles per instruction (CPI) is a good metric for uniprocessor, but not necessarily for multi processor. In multiprocessors, some core may have to wait for others, to have a specific information or whatever, so when the core wait, it will just do some spinning threads which is very fast $\implies$ a low CPI! But in fact, you don't execute the program particularly faster.
...
\subsection{Amdahl's law}
    \begin{tcolorbox}[rouge]
        The amdahl's law shows that we have to improve efficiency of what take the most of the time in our code execution. 
    ...
    \begin{tcolorbox}[gris]
         \begin{itemize}[left=10pt, label=\textbullet]
            ...
            \item We can know by what we are limited by taking 
            \[min\left(\text{peak FLOPs/sec }\space, \space \text{bandwidth}\cdot \text{intensity}\right).\]
            It means that
...
void vecAdd(int N) {
    ...
    \subsection{Tensor cores}
        We invented specialized GPU for deep neural network (DNN), where the main operation is the matrix multiplication.
        \begin{center}
        \begin{minipage}{0.45\textwidth}
