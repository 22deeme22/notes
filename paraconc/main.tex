...
\subsection{Parallel reduction}
    ...
    \end{tcolorbox}
    Reduction has a low ariithmetic intensity, all input elements are loaded from memory, so the goal is to reduce global memory accesses. Since partial sums can be reused, we can load to shared memory first.
    \begin{tcolorbox}[rouge]
        ...
        \begin{itemize}[left=10pt, label=\textbullet]
            ...
            \item Desactivate half of the threads at the end of each step. 
...
__global__ void hist_calc(char* data, unsigned int length, unsigned int* histogram) {
    ...
    \end{Ccode}
    \end{codeCenter}
    The problem here is that we have an atomic operation that access global memory at each loop, so it's really inefficient. To solve this, we'll reduce srialization through privatization. It means that each threadblock will keeps their own copy of histogram in the shared memory and the merge private copies in the global memory in phase 2.
    \begin{center}
...
__device__ void readXY()
{
    ...
    \end{minipage}
    \end{center}
    The \texttt{\_\_threadfence} of thread 1 ensures that \texttt{Y=20} muste be observed after \texttt{X=10}. The \texttt{\_\_threadfence} of thread 2 ensures that the two loads are not reordered, so \texttt{Y} is read first and then \texttt{X}.
    \begin{tcolorbox}[rouge]
